-- ═══════════════════════════════════════════════════════════════
-- GRUPO SRSM — DADOS DE DEMONSTRAÇÃO (IDEMPOTENTE)
-- Pode ser rodado múltiplas vezes sem duplicar dados.
-- Clientes/produtos/categorias: upsert (atualiza campos faltantes).
-- Vendas/empréstimos/parcelas/movimentações: insere só se não existe.
-- ═══════════════════════════════════════════════════════════════

DO $$
DECLARE
  v_emp UUID; v_fac UUID;
  v_cat_sofa UUID; v_cat_mesa UUID; v_cat_cama UUID; v_cat_arm UUID;
  v_forn UUID;
  ...
  v_ec1 UUID; v_ec2 UUID; v_ec3 UUID; v_ec4 UUID; v_ec5 UUID; v_ec6 UUID;
  v_v1 UUID; v_v2 UUID; v_v3 UUID; v_v4 UUID; v_v5 UUID; v_v6 UUID;
  v_v7 UUID; v_v8 UUID;
  v_fc1 UUID; v_fc2 UUID; v_fc3 UUID; v_fc4 UUID; v_fc5 UUID; v_fc6 UUID;
  v_fc7 UUID; v_fc8 UUID;
...
BEGIN
  ...
  IF v_emp IS NULL OR v_fac IS NULL THEN
    RAISE EXCEPTION 'Empresas não encontradas. Verifique a tabela empresas.';
  END IF;
  ...
  VALUES (v_emp, 'emporio-dos-moveis', 'Empório dos Móveis',
    'Os melhores móveis com o melhor preço da região. Qualidade e conforto para sua casa.',
    '64999990001', '@emporiodosmoveis', TRUE, FALSE, TRUE)
  ...
  -- CATEGORIAS — upsert + get ID real
  INSERT INTO categorias_produto (id, empresa_id, nome, slug, icone, ordem, descricao) VALUES
    (gen_random_uuid(), v_emp, 'Sofás e Poltronas',   'sofas-poltronas',  '🛋️', 1, 'Sofás, poltronas e chaises para sala de estar'),
    (gen_random_uuid(), v_emp, 'Mesas e Cadeiras',    'mesas-cadeiras',   '🪑', 2, 'Mesas de jantar, escritório e cadeiras diversas'),
    (gen_random_uuid(), v_emp, 'Camas e Colchões',    'camas-colchoes',   '🛏️', 3, 'Camas box, colchões e cabeceiras'),
    (gen_random_uuid(), v_emp, 'Armários e Roupeiros','armarios-roupeiros','🚪', 4, 'Guarda-roupas, armários e cômodas')
  ON CONFLICT (empresa_id, slug) DO UPDATE SET
    nome = EXCLUDED.nome, descricao = EXCLUDED.descricao, icone = EXCLUDED.icone;

  SELECT id INTO v_cat_sofa FROM categorias_produto WHERE empresa_id = v_emp AND slug = 'sofas-poltronas';
  SELECT id INTO v_cat_mesa FROM categorias_produto WHERE empresa_id = v_emp AND slug = 'mesas-cadeiras';
  SELECT id INTO v_cat_cama FROM categorias_produto WHERE empresa_id = v_emp AND slug = 'camas-colchoes';
  SELECT id INTO v_cat_arm  FROM categorias_produto WHERE empresa_id = v_emp AND slug = 'armarios-roupeiros';

  -- FORNECEDOR — check by nome
  SELECT id INTO v_forn FROM fornecedores WHERE empresa_id = v_emp AND nome = 'Móveis Brasil Atacado Ltda' LIMIT 1;
  IF v_forn IS NULL THEN
    INSERT INTO fornecedores (empresa_id, nome, cnpj, telefone, email, endereco, cidade, estado, contato, observacoes)
    VALUES (v_emp, 'Móveis Brasil Atacado Ltda', '12.345.678/0001-90', '64933334444',
  ...
  -- PRODUTOS — upsert + get ID real
  INSERT INTO produtos (id, empresa_id, categoria_id, fornecedor_id, nome, sku, descricao, descricao_curta,
    preco, preco_custo, estoque, estoque_minimo, unidade, peso, destaque, disponivel_catalogo, status, tags) VALUES
    (gen_random_uuid(), v_emp, v_cat_sofa, v_forn, 'Sofá 3 Lugares Retrátil Suede',   'SOF-001',
    ...
    (gen_random_uuid(), v_emp, v_cat_sofa, v_forn, 'Poltrona Decorativa Veludo Azul', 'SOF-002',
    ...
    (gen_random_uuid(), v_emp, v_cat_sofa, v_forn, 'Sofá Canto 5 Lugares com Chaise', 'SOF-003',
    ...
    (gen_random_uuid(), v_emp, v_cat_mesa, v_forn, 'Mesa de Jantar 6 Lugares MDF',   'MES-001',
    ...
    (gen_random_uuid(), v_emp, v_cat_mesa, v_forn, 'Cadeira Estofada Giratória Slim','MES-002',
    ...
    (gen_random_uuid(), v_emp, v_cat_mesa, v_forn, 'Mesa de Centro Redonda Vidro',   'MES-003',
    ...
    (gen_random_uuid(), v_emp, v_cat_cama, v_forn, 'Cama Box Casal Queen 1,58m',     'CAM-001',
    ...
    (gen_random_uuid(), v_emp, v_cat_cama, v_forn, 'Colchão Molas Ensacadas Casal',  'CAM-002',
    ...
    (gen_random_uuid(), v_emp, v_cat_cama, v_forn, 'Cabeceira Estofada King 2,00m',  'CAM-003',
    ...
    (gen_random_uuid(), v_emp, v_cat_arm,  v_forn, 'Guarda-Roupa 6 Portas Espelho',  'ARM-001',
    ...
    (gen_random_uuid(), v_emp, v_cat_arm,  v_forn, 'Cômoda 5 Gavetas Amadeirada',    'ARM-002',
    ...
    (gen_random_uuid(), v_emp, v_cat_arm,  v_forn, 'Armário Multiuso 4 Portas',      'ARM-003',
  ...
  ON CONFLICT (empresa_id, sku) DO UPDATE SET
    descricao = EXCLUDED.descricao, descricao_curta = EXCLUDED.descricao_curta,
    peso = EXCLUDED.peso, tags = EXCLUDED.tags, destaque = EXCLUDED.destaque,
    preco_custo = EXCLUDED.preco_custo;

  SELECT id INTO v_p1  FROM produtos WHERE empresa_id = v_emp AND sku = 'SOF-001';
  SELECT id INTO v_p2  FROM produtos WHERE empresa_id = v_emp AND sku = 'SOF-002';
  SELECT id INTO v_p3  FROM produtos WHERE empresa_id = v_emp AND sku = 'SOF-003';
  SELECT id INTO v_p4  FROM produtos WHERE empresa_id = v_emp AND sku = 'MES-001';
  SELECT id INTO v_p5  FROM produtos WHERE empresa_id = v_emp AND sku = 'MES-002';
  SELECT id INTO v_p6  FROM produtos WHERE empresa_id = v_emp AND sku = 'MES-003';
  SELECT id INTO v_p7  FROM produtos WHERE empresa_id = v_emp AND sku = 'CAM-001';
  SELECT id INTO v_p8  FROM produtos WHERE empresa_id = v_emp AND sku = 'CAM-002';
  SELECT id INTO v_p9  FROM produtos WHERE empresa_id = v_emp AND sku = 'CAM-003';
  SELECT id INTO v_p10 FROM produtos WHERE empresa_id = v_emp AND sku = 'ARM-001';
  SELECT id INTO v_p11 FROM produtos WHERE empresa_id = v_emp AND sku = 'ARM-002';
  SELECT id INTO v_p12 FROM produtos WHERE empresa_id = v_emp AND sku = 'ARM-003';

  -- CLIENTES EMPÓRIO — upsert (atualiza campos opcionais faltantes) + get ID real
  INSERT INTO clientes_emporio (id, empresa_id, nome, cpf, rg, data_nascimento, telefone, telefone2, email,
    endereco, numero, complemento, bairro, cidade, estado, cep,
    total_compras, valor_total_compras, ultima_compra, observacoes, status)
  VALUES
    (gen_random_uuid(), v_emp, 'Ana Paula Rodrigues',  '111.222.333-44','1234567 SSP/GO','1988-03-15',
    ...
    (gen_random_uuid(), v_emp, 'Carlos Eduardo Lima',  '222.333.444-55','2345678 SSP/GO','1975-07-22',
    ...
    (gen_random_uuid(), v_emp, 'Fernanda Costa Silva', '333.444.555-66','3456789 SSP/GO','1992-11-08',
    ...
    (gen_random_uuid(), v_emp, 'Roberto Alves Souza',  '444.555.666-77','4567890 SSP/GO','1969-05-30',
    ...
    (gen_random_uuid(), v_emp, 'Mariana Tavares',      '555.666.777-88','5678901 SSP/GO','1995-01-14',
    ...
    (gen_random_uuid(), v_emp, 'José Henrique Nunes',  '666.777.888-99','6789012 SSP/GO','1983-09-19',
  ...
  ON CONFLICT (cpf) DO UPDATE SET
    ...
    cep                 = EXCLUDED.cep,
    observacoes         = COALESCE(EXCLUDED.observacoes, clientes_emporio.observacoes),
    email               = COALESCE(EXCLUDED.email, clientes_emporio.email);

  SELECT id INTO v_ec1 FROM clientes_emporio WHERE cpf = '111.222.333-44' AND empresa_id = v_emp;
  SELECT id INTO v_ec2 FROM clientes_emporio WHERE cpf = '222.333.444-55' AND empresa_id = v_emp;
  SELECT id INTO v_ec3 FROM clientes_emporio WHERE cpf = '333.444.555-66' AND empresa_id = v_emp;
  SELECT id INTO v_ec4 FROM clientes_emporio WHERE cpf = '444.555.666-77' AND empresa_id = v_emp;
  SELECT id INTO v_ec5 FROM clientes_emporio WHERE cpf = '555.666.777-88' AND empresa_id = v_emp;
  SELECT id INTO v_ec6 FROM clientes_emporio WHERE cpf = '666.777.888-99' AND empresa_id = v_emp;

  -- VENDAS — insere só se a empresa não tiver nenhuma venda ainda
  IF NOT EXISTS (SELECT 1 FROM vendas WHERE empresa_id = v_emp LIMIT 1) THEN
    v_v1 := gen_random_uuid(); v_v2 := gen_random_uuid(); v_v3 := gen_random_uuid();
    v_v4 := gen_random_uuid(); v_v5 := gen_random_uuid(); v_v6 := gen_random_uuid();
    v_v7 := gen_random_uuid(); v_v8 := gen_random_uuid();
  ...
  -- CONTAS PAGAR EMPÓRIO — insere só se não existir nenhuma
  IF NOT EXISTS (SELECT 1 FROM contas_pagar WHERE empresa_id = v_emp LIMIT 1) THEN
  ...
  -- MOVIMENTAÇÕES CAIXA EMPÓRIO — insere só se não existir nenhuma
  IF NOT EXISTS (SELECT 1 FROM movimentacoes_caixa WHERE empresa_id = v_emp LIMIT 1) THEN
  ...
  -- CLIENTES FACTORING — upsert (atualiza todos os campos opcionais) + get ID real
  INSERT INTO clientes_factoring (id, empresa_id, nome, cpf, rg, orgao_emissor, data_nascimento,
    ...
    total_emprestimos, valor_total_emprestado, ultima_operacao, observacoes, status)
  VALUES
    (gen_random_uuid(),v_fac,'Marcos Antônio Ferreira','101.202.303-10','1234567','SSP/GO','1979-04-12',
    ...
    (gen_random_uuid(),v_fac,'Luciana Borges Melo','202.303.404-20','2345678','SSP/GO','1987-09-25',
    ...
    (gen_random_uuid(),v_fac,'Paulo Sérgio Araújo','303.404.505-30','3456789','SSP/GO','1968-01-30',
    ...
    (gen_random_uuid(),v_fac,'Tatiana Ramos Vieira','404.505.606-40','4567890','SSP/GO','1990-06-18',
    ...
    (gen_random_uuid(),v_fac,'Elias Gonçalves Pinto','505.606.707-50','5678901','SSP/GO','1983-12-03',
    ...
    (gen_random_uuid(),v_fac,'Simone Dias Cardoso','606.707.808-60','6789012','SSP/GO','1995-08-22',
    ...
    (gen_random_uuid(),v_fac,'André Luiz Barbosa','707.808.909-70','7890123','SSP/GO','1977-03-09',
    ...
    (gen_random_uuid(),v_fac,'Cristina Moura Santos','808.909.010-80','8901234','SSP/GO','1982-05-14',
  ...
  ON CONFLICT (cpf) DO UPDATE SET
    ...
    pix                  = EXCLUDED.pix,
    observacoes          = COALESCE(EXCLUDED.observacoes, clientes_factoring.observacoes),
    email                = COALESCE(EXCLUDED.email, clientes_factoring.email);

  SELECT id INTO v_fc1 FROM clientes_factoring WHERE cpf = '101.202.303-10' AND empresa_id = v_fac;
  SELECT id INTO v_fc2 FROM clientes_factoring WHERE cpf = '202.303.404-20' AND empresa_id = v_fac;
  SELECT id INTO v_fc3 FROM clientes_factoring WHERE cpf = '303.404.505-30' AND empresa_id = v_fac;
  SELECT id INTO v_fc4 FROM clientes_factoring WHERE cpf = '404.505.606-40' AND empresa_id = v_fac;
  SELECT id INTO v_fc5 FROM clientes_factoring WHERE cpf = '505.606.707-50' AND empresa_id = v_fac;
  SELECT id INTO v_fc6 FROM clientes_factoring WHERE cpf = '606.707.808-60' AND empresa_id = v_fac;
  SELECT id INTO v_fc7 FROM clientes_factoring WHERE cpf = '707.808.909-70' AND empresa_id = v_fac;
  SELECT id INTO v_fc8 FROM clientes_factoring WHERE cpf = '808.909.010-80' AND empresa_id = v_fac;

  -- REFERÊNCIAS — insere só se não existir (por cliente + telefone)
  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc1,'Maria Aparecida Ferreira','Esposa',  '64988111111','Confirmou emprego e residência do Marcos.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc1 AND telefone = '64988111111');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc1,'José Carlos Ferreira','Pai','64988111112','Reside no mesmo bairro. Confirmou dados.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc1 AND telefone = '64988111112');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc2,'Rafael Augusto Melo','Esposo','64988222221','Professor. Confirmou dados da Luciana.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc2 AND telefone = '64988222221');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc3,'Ana Paula Araújo','Esposa','64988333331','Confirmou propriedade rural.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc3 AND telefone = '64988333331');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc4,'Roberta Ramos','Mãe','64988444441','Confirmou residência e emprego.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc4 AND telefone = '64988444441');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc5,'Renata Gonçalves','Ex-esposa','64988555551','Referência limitada. Contato dificultado.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc5 AND telefone = '64988555551');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc6,'Lucas Cardoso','Irmão','64988666661','Confirmou residência mas não quis dar mais informações.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc6 AND telefone = '64988666661');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc7,'Marcia Luiz Barbosa','Esposa','64988777771','Confirmou todos os dados. Muito colaborativa.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc7 AND telefone = '64988777771');

  INSERT INTO referencias_cliente_factoring (cliente_id, nome, parentesco, telefone, observacoes)
  SELECT v_fc8,'Roberto Santos','Esposo','64988888881','Engenheiro. Confirmou renda e residência da Cristina.'
  WHERE NOT EXISTS (SELECT 1 FROM referencias_cliente_factoring WHERE cliente_id = v_fc8 AND telefone = '64988888881');

  -- EMPRÉSTIMOS — upsert (atualiza observacoes e garantias) + get ID real
  INSERT INTO emprestimos (id, empresa_id, numero_contrato, cliente_id, valor_principal,
    ...
    observacoes, garantias, status)
  VALUES
    (gen_random_uuid(),v_fac,'SRS-2026-00001',v_fc1,10000.00,3.50,'mensal',12,
    ...
    (gen_random_uuid(),v_fac,'SRS-2026-00002',v_fc2,5000.00,3.50,'mensal',6,
    ...
    (gen_random_uuid(),v_fac,'SRS-2026-00003',v_fc3,8000.00,3.50,'mensal',10,
    ...
    (gen_random_uuid(),v_fac,'SRS-2026-00004',v_fc4,4000.00,3.50,'mensal',4,
    ...
    (gen_random_uuid(),v_fac,'SRS-2026-00005',v_fc5,3000.00,3.50,'mensal',6,
    ...
    (gen_random_uuid(),v_fac,'SRS-2026-00006',v_fc6,3000.00,3.50,'mensal',6,
     554.23,3325.38,325.38,0.00,3325.38,
  ...
  ON CONFLICT (empresa_id, numero_contrato) DO UPDATE SET
    observacoes = EXCLUDED.observacoes,
    garantias   = EXCLUDED.garantias;

  SELECT id INTO v_e1 FROM emprestimos WHERE empresa_id = v_fac AND numero_contrato = 'SRS-2026-00001';
  SELECT id INTO v_e2 FROM emprestimos WHERE empresa_id = v_fac AND numero_contrato = 'SRS-2026-00002';
  SELECT id INTO v_e3 FROM emprestimos WHERE empresa_id = v_fac AND numero_contrato = 'SRS-2026-00003';
  SELECT id INTO v_e4 FROM emprestimos WHERE empresa_id = v_fac AND numero_contrato = 'SRS-2026-00004';
  SELECT id INTO v_e5 FROM emprestimos WHERE empresa_id = v_fac AND numero_contrato = 'SRS-2026-00005';
  SELECT id INTO v_e6 FROM emprestimos WHERE empresa_id = v_fac AND numero_contrato = 'SRS-2026-00006';

  -- PARCELAS EMPRÉSTIMO — insere só se o empréstimo ainda não tiver parcelas
  IF NOT EXISTS (SELECT 1 FROM parcelas_emprestimo WHERE emprestimo_id = v_e1 LIMIT 1) THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM parcelas_emprestimo WHERE emprestimo_id = v_e2 LIMIT 1) THEN
    INSERT INTO parcelas_emprestimo (empresa_id, emprestimo_id, cliente_id, numero_parcela, total_parcelas,
      valor, valor_principal, valor_juros, saldo_devedor_antes, saldo_devedor_apos,
      valor_pago, data_vencimento, data_pagamento, tipo_pagamento, multa, juros_mora, observacoes, status) VALUES
  ...
  IF NOT EXISTS (SELECT 1 FROM parcelas_emprestimo WHERE emprestimo_id = v_e3 LIMIT 1) THEN
    INSERT INTO parcelas_emprestimo (empresa_id, emprestimo_id, cliente_id, numero_parcela, total_parcelas,
      valor, valor_principal, valor_juros, saldo_devedor_antes, saldo_devedor_apos,
      valor_pago, data_vencimento, data_pagamento, tipo_pagamento, multa, juros_mora, observacoes, status) VALUES
  ...
  IF NOT EXISTS (SELECT 1 FROM parcelas_emprestimo WHERE emprestimo_id = v_e4 LIMIT 1) THEN
    INSERT INTO parcelas_emprestimo (empresa_id, emprestimo_id, cliente_id, numero_parcela, total_parcelas,
      valor, valor_principal, valor_juros, saldo_devedor_antes, saldo_devedor_apos,
      valor_pago, data_vencimento, data_pagamento, tipo_pagamento, multa, juros_mora, observacoes, status) VALUES
  ...
  IF NOT EXISTS (SELECT 1 FROM parcelas_emprestimo WHERE emprestimo_id = v_e5 LIMIT 1) THEN
    INSERT INTO parcelas_emprestimo (empresa_id, emprestimo_id, cliente_id, numero_parcela, total_parcelas,
      valor, valor_principal, valor_juros, saldo_devedor_antes, saldo_devedor_apos,
      valor_pago, data_vencimento, data_pagamento, tipo_pagamento, multa, juros_mora, observacoes, status) VALUES
  ...
  IF NOT EXISTS (SELECT 1 FROM parcelas_emprestimo WHERE emprestimo_id = v_e6 LIMIT 1) THEN
    INSERT INTO parcelas_emprestimo (empresa_id, emprestimo_id, cliente_id, numero_parcela, total_parcelas,
      valor, valor_principal, valor_juros, saldo_devedor_antes, saldo_devedor_apos,
      valor_pago, data_vencimento, data_pagamento, tipo_pagamento, multa, juros_mora, observacoes, status) VALUES
  ...
  -- CONTAS PAGAR FACTORING — insere só se não existir nenhuma
  IF NOT EXISTS (SELECT 1 FROM contas_pagar WHERE empresa_id = v_fac LIMIT 1) THEN
  ...
  -- MOVIMENTAÇÕES CAIXA FACTORING — insere só se não existir nenhuma
  IF NOT EXISTS (SELECT 1 FROM movimentacoes_caixa WHERE empresa_id = v_fac LIMIT 1) THEN
    INSERT INTO movimentacoes_caixa (empresa_id, tipo, categoria, descricao, valor, data_movimentacao, observacoes) VALUES
  ...
  -- NOTIFICAÇÕES — insere só se não existir nenhuma
  IF NOT EXISTS (SELECT 1 FROM notificacoes_log WHERE empresa_id = v_fac LIMIT 1) THEN
  ...
  RAISE NOTICE 'SUCESSO! Dados de demonstração inseridos/atualizados.';
