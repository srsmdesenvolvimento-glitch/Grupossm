-- ===================================================================
-- MIGRATION: WHATSAPP INTEGRATION, AUTOMATIONS & IMPROVEMENTS
-- ===================================================================
...
-- 2. Add whatsapp_settings column to config_factoring with default templates and hora_envio
...
-- 3. Add whatsapp_message_id to notificacoes_log to track delivery status from webhooks
ALTER TABLE notificacoes_log ADD COLUMN IF NOT EXISTS whatsapp_message_id VARCHAR(100);

-- Create index for performance on webhook lookups
CREATE INDEX IF NOT EXISTS idx_notificacoes_log_whatsapp_msg_id ON notificacoes_log(whatsapp_message_id);

-- Update any existing factoring configs that don't have the hora_envio field inside whatsapp_settings
